...
classdef NullModels < handle
    ...
    methods(Static)
        ...
        function random_matrix = EQUIPROBABLE(MatrixOrSizeRows, n_cols, ones)
        % EQUIPROBABLE - Create a random matrix with equiprobable probabilities
        ...
        function random_matrix = AVERAGE(n_rows, n_cols, probRows,probCols)
        ...
        %      p_ij = 1/2 * ( probRows(i) + probCols(j))
            ...
            hasNull = 1;
            while(hasNull)
                random_matrix = rand(n_rows,n_cols) <= mat;
