...
% 2013> for an example). In other words, if wee focus at the global scale
...
% Performing this kind of analysis may be exhaustive, specially if nothing
% is to be found. However, |BiMat| provides a the tools for performing a
...
% the same class may be clustered inside the same module). 
%
%% Example: Moebus Meta-scale analysis
% In this example we will describe how to perform this kind of statistical
% analysis in a network that is large enought. This network is composed
...
%
% We can now perform the standar modularity and nestedness calculation.
% For modularity:
...
%
% It it evident that the entire network is modular (a result 
